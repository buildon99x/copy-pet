...
pub fn t(lang: Lang, msg: Msg) -> &'static str {
    ...
    match (msg, lang) {
        ...
        (MenuTheme, Lang::En) => "Theme",
        (MenuTheme, Lang::Ko) => "테마",
        (ThemeDark, Lang::En) => "Dark",
        (ThemeDark, Lang::Ko) => "어두운 톤",
        (ThemeLight, Lang::En) => "Light",
        (ThemeLight, Lang::Ko) => "밝은 톤",
        ...
        (ExpToday, Lang::Ko) => "오늘",
        (ExpAfterCopy, Lang::En) => "After copy",
        (ExpAfterCopy, Lang::Ko) => "복사 후",
        (MenuCapture, Lang::En) => "Capture",
        (MenuCapture, Lang::Ko) => "캡처",
        ...
        (ActCopy, Lang::Ko) => "복사",
        (ActQuickCopy, Lang::En) => "Quick Copy",
        (ActQuickCopy, Lang::Ko) => "바로 복사",
        (ActPin, Lang::En) => "Pin",
        (ActPin, Lang::Ko) => "고정",
        (ActUnpin, Lang::En) => "Unpin",
        (ActUnpin, Lang::Ko) => "고정 해제",
        (ActEditNote, Lang::En) => "Edit Note",
        (ActEditNote, Lang::Ko) => "메모",
        (ActDelete, Lang::En) => "Delete",
        (ActDelete, Lang::Ko) => "삭제",
        (ActOpenSource, Lang::En) => "Open Source",
        (ActOpenSource, Lang::Ko) => "출처 열기",
        (DetailId, Lang::En) => "ID",
        (DetailId, Lang::Ko) => "ID",
        (ClipInfo, Lang::En) => "CLIP INFO",
        (ClipInfo, Lang::Ko) => "클립 정보",
        (AllSources, Lang::En) => "All Sources",
        (AllSources, Lang::Ko) => "모든 출처",
        (ExpTip, Lang::En) => "Ctrl+0–9 quick-copies the top 10. Ctrl+Z undoes a delete.",
        (ExpTip, Lang::Ko) => "Ctrl+0–9로 상위 10개 바로 복사, Ctrl+Z로 삭제 취소.",
...
mod tests {
    ...
    fn every_message_has_both_translations() {
        ...
        let all = [
            ...
            SizeLarge, MenuTheme, ThemeDark, ThemeLight,
            MenuAccessory, AccNone, MenuSound, SoundOff, SoundEvents, SoundAll,
            ...
            ExpToday, ExpAfterCopy, MenuCapture, CaptureRunning, CapturePaused,
            ActCopy, ActQuickCopy, ActPin, ActUnpin, ActEditNote, ActDelete, ActOpenSource,
            DetailCreated, DetailSource, DetailSize, DetailType, DetailEmpty,
            DetailId, ClipInfo, ExpTip, AllSources,
        ];
        for msg in all {
