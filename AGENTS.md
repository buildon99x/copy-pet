...
3. **No new heavy dependencies** without an ADR. The whole point is a small
   binary with a handful of crates and no asset pipeline (icon, font — incl.
   the vector-Hangul font — and sounds are generated from code).
...
src/
  ...
  platform/
    ...
    windows.rs         native Win32 layered window + LL hooks + clipboard
                       listener + Ctrl+Shift+V hotkey + Shell tray
...
- `all(windows, not(feature = "portable"))` → **native Win32** (default on
  ...
  app name + real icon extraction), global Ctrl+Shift+V hotkey, Shell tray
  with full context menu, HKCU autostart.
